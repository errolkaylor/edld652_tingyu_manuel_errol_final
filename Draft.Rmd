...
ggplot(ach,aes(fct_reorder(LEAID,ALL_RLA00NUMVALID),ALL_RLA00NUMVALID))+
  geom_col(fill="#00ff7b")+ # this color is so bright! Hard to distinguish between bars.
  coord_flip()+
  theme_classic() 
...
ggplot(ach,aes(LEAID,LEP_RLA00NUMVALID))+
  geom_col(fill="#00ffe1")+
  theme_classic() 
# very hard to read the axis on these plots. Consider renaming or changing?
```
...
#district_membership <- as_tibble(get_data("NCES_CCD_nonfiscal_district_2017_2021_membership")) # I tried to run this twice and it took some serious time (bless your soul and patience). In the end, I first got an error that it failed to open a local file, so crushing. The next time the error was due to size 'Error: cannot allocate vector of size 4.0 Gb'. Hope it works with yours but might want to look into. Because of it I couldn't run the rest of the code using this piece here.
head(district_el)
...
or_district_el <- district_el %>% 
  ...
  select(LEAID,
         SCHOOL_YEAR,
         LEP_COUNT) # I like this process of making such a large file and narrowing onto one state. Clever!

or_district_membership <- district_membership %>% # couldn't run this due to district_membership from code earlier not working.
  filter(STATENAME == "OREGON")
...
leaflet(or_district_data) %>%
  ...
  addPolygons(color = ~pal(full_dataset$LEP_COUNT),
              weight = 0.5,
              fillOpacity = 1) 
# I could not run line 95 but everything else in this ran. This visual is so cool! I did not even know you can zoom in and out in R.
```
...
or_enroll_total <- or_district_membership %>%
  ...
  # this is what I can't run because unable to get 'or_district_membership' since couldn't get 'district_membership'. Code looks clean and easy to follow though!
...
```{r }
# and I can't run this because of the previous code that is broken. I tried looking back if maybe it was something I could change but 
or_enroll_total%>%
