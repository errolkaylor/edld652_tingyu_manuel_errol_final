...
ggplot(ach, aes(fct_reorder(LEAID, ALL_RLA00NUMVALID), #Amy: Added a line break here to keep code within 80 characters
                ALL_RLA00NUMVALID, fill = LEP_RLA00NUMVALID)) +
  geom_col() +
  coord_flip() +
  theme_classic()
...
head(district_el)
district_membership <- as_tibble(read_parquet(here( "district-membership-17-21.parquet"))) #Amy: I couldn't get this to work.
...
#Amy: This wouldn't run due to the originating parquet file not loading
...
or_enroll_total <- or_district_membership %>%
  ...
  select(leaid, lea_name, student_count, student_white_count, 
         white_pct,schools_dli, lep_count) %>%
...
or_enroll_total %>%
  ...
  geom_point(data = filter(or_enroll_total, schools_dli > 0 & !is.na(dli_cat)),
             aes(el_pct, y = white_pct, 
                 size = lep_count, color = as.factor(dli_cat))) +
  geom_text_repel(data = filter(or_enroll_total,el_pct > 10 
                                & lep_count > 750 & schools_dli == 0),
                  aes(label = lea_name), 
...
# create the quadrants
  ...
  # quadrant labels
  annotate("text", x = 5, y = 95, alpha = 0.35, 
           label = "High White, Low EL Districts") +
  annotate("text", x = 20, y = 95, alpha = 0.35, label = " ") +
  annotate("text", x = 20, y = 15, alpha = 0.35, 
           label = "Low White, High EL Districts\n\ (Candiates for DLI programs)") +
