...
ggplot(ach,aes(fct_reorder(LEAID,ALL_RLA00NUMVALID),ALL_RLA00NUMVALID))+
  ...
  coord_flip()+
  theme_classic() # stay away from green for colorblind individuals
...
ggplot(ach,aes(LEAID,LEP_RLA00NUMVALID))+
  geom_col(fill="#00ffe1")+
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) # this is a start, but you will need to relevel to make them understandable.
  
# Not really sure what the end goal is here. If you are going to keep this, you will need to make sure that all of the LEAID numbers are visible or noted in some way. 
```
...
head(district_el)
district_membership <- as_tibble(read_parquet(here( "district-membership-17-21.parquet"))) # this parquet is not in the data folder. Cannot run this.
...
or_district_el_directory <- 
  left_join(or_district_directory,
            or_district_el,
            by="LEAID") # cannot run this because the parquet file needed is not in data folder
...
# Though I cannot load the plot since the parquet file is missing. From the code, it looks like the annotating will be a great piece! Also, I really like the descriptive title. It helps me understand without actually viewing the plot at all!
