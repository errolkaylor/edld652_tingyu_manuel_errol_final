...
DATAf <- DATA %>% 
  mutate(rla_ach = case_when(LEP_RLA00PCTPROF <50 ~ "less than 50%",
                             LEP_RLA00PCTPROF >= 50 ~"at least 50%"))
```
...
DATAf %>%
  ggplot(aes(x = el_pct, y=students_color_pct)) +
  geom_point(aes(size = schools_dli), color = "grey80")+ 
    geom_point(aes(color=rla_ach))+
    theme_minimal() +
         labs(title = "There is a high concentration of better performance in predominantly white districts", 
              y = "Percent of color", 
              x = "Percent EL of total enrollment",
              color = "Percentage of LEP that scored at or above proficient",
              size="Dual language immersion programs")+
